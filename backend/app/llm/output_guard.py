...
def _numbered_lists_to_bullets_in_precautions(text: str) -> str:
    """## 유의사항 구간의 1. 2. 목록을 - 불릿으로 변환."""
    ...
    for line in text.splitlines():
        ...
        if re.match(r"^##\s+", stripped):
            ...
            in_precautions = title in ("유의사항", "유의 사항") or title.replace(" ", "") == "유의사항"
            out.append(line)
            continue
        if in_precautions and re.match(r"^\d+[.)]\s+", stripped):
...
def _strip_evidence_section(text: str) -> str:
    ...
    out = re.sub(r"\n\[(?:근거\s*출처|참고\s*문헌|출처)\][\s\S]*$", "", out, flags=re.IGNORECASE)
    return out.strip()
...
def postprocess_answer(text: str) -> str:
    ...
    text = _strip_evidence_section(text)
    text = _numbered_lists_to_bullets_in_precautions(text)
...
_JOINT_SECTION_NOISE_RE = re.compile(
    r"^(?:#+\s*)?(?:개요|핵심\s*원칙|운용\s*절차|지휘관\s*고려\s*사항|유의\s*사항|근거)\s*[:：]?\s*$",
    re.IGNORECASE,
)


def _compact_joint_bullet_body(body: str, *, max_chars: int = 96) -> str:
    """불릿 본문에서 소제목·목록 제거 후 첫 문장만 (1~2줄 UI용)."""
    text = body or ""
    text = re.sub(r"^#+\s*\S+\s*", "", text, flags=re.M)
    text = re.sub(r"^[-*•]\s*", "", text, flags=re.M)
    text = _JOINT_SECTION_NOISE_RE.sub("", text)
    text = re.sub(r"\s+", " ", text).strip()
    text = re.sub(r"^\(.*문장.*\)\s*", "", text)
    text = re.sub(r"^(?:합동|공통)\s*비교\s*종합\s*[-–:：]?\s*", "", text, flags=re.I)
    text = re.sub(
    ...
    if not text or re.search(r"비교\s*종합|핵심\s*\d+\s*문장", text):
        return ""
    if re.fullmatch(r"(?:개요|핵심|운용|지휘관|유의|근거)", text, re.I):
        return ""
    parts = [p.strip() for p in re.split(r"(?<=[.!?。])\s+", text) if p.strip()]
    one = parts[0] if parts else text
...
def polish_joint_summary(raw: str, *, max_lines: int = 5, max_chars_per_line: int = 96) -> str:
    """3군 비교 요약 전용 — 군별 1문장·짧은 불릿만."""
    if not raw:
        return ""
    ...
    for chunk in re.split(r"\n+", text):
        chunk = chunk.strip()
        if not chunk or _JOINT_SECTION_NOISE_RE.match(chunk):
            continue
        chunk = re.sub(r"^[-*•\d.]+\s*", "", chunk)
        chunk = _JOINT_PREFIX_RE.sub("", chunk)
        m = re.match(r"^(공통|육군|해군|공군|합동)\s*[:：]\s*(.+)$", chunk, re.I)
        m_en = re.match(r"^(육군|해군|공군)은\s*(.+)$", chunk) if not m else None
        if m:
            label = m.group(1)
            ...
            chunk = f"{m_en.group(1)}: {m_en.group(2).strip()}"
    ...
    if len(lines) == 1 and len(lines[0]) > max_chars_per_line:
        one = lines[0]
        for sep in (" 육군:", " 해군:", " 공군:", " 육군은", " 해군은", " 공군은", "②", "③", "④", "⑤"):
            if sep in one:
                parts = re.split(
                    r"(?=\s*(?:육군|해군|공군)(?:은|:|：|\s))",
                    one,
    ...
    for ln in lines:
        ...
        if not re.match(r"^(공통|육군|해군|공군|합동)\s*:", ln):
            label_guess = ("육군", "해군", "공군")[min(len(polished), 2)]
            ln = f"{label_guess}: {ln}"
        m_ln = re.match(r"^(공통|육군|해군|공군|합동)\s*[:：]\s*(.+)$", ln)
        if m_ln:
            compact = _compact_joint_bullet_body(m_ln.group(2).strip(), max_chars=max_chars_per_line)
            if compact:
                ln = f"{m_ln.group(1)}: {compact}"
            else:
                continue
        elif len(ln) > max_chars_per_line:
    ...
    def _sort_key(line: str) -> int:
        ...
        order = {"육군:": 0, "해군:": 1, "공군:": 2, "공통:": 3, "합동:": 4}
    ...
    branch_only = [
        ln
        for ln in polished
        if re.match(r"^(육군|해군|공군)\s*:", ln)
    ]
    if branch_only:
        polished = branch_only[:3]

    bullets: list[str] = []
    for ln in polished[: max(max_lines, 3)]:
        m = re.match(r"^(공통|육군|해군|공군|합동)\s*[:：]\s*(.+)$", ln)
        if m:
            label = m.group(1)
            body = _compact_joint_bullet_body(m.group(2).strip(), max_chars=max_chars_per_line)
            if body:
