...
import config
from llm._utils import answer_has_degeneration, answer_lacks_korean, count_hangul, run_async
from llm.output_guard import polish_joint_summary
...
def synthesize_joint_branch_comparison(
    ...
    *,
    ...
    """육·해·공 RAG 답변을 5줄 이내 합동 비교·종합으로 압축."""
    blocks: list[str] = []
    ...
    user_prompt = (
        ...
        "형식: ## 합동 비교 종합 + - **공통:** / **육군:** / **해군:** / **공군:** / **합동:** 불릿 5개.\n\n"
        + "\n\n".join(blocks)
    )
    try:
        client = get_llm_client()
    ...
    for b in ("army", "navy", "air_force"):
        label = _BRANCH_KO[b]
        snippet = (answers_by_branch.get(b) or "").replace("\n", " ").strip()
        first = re.split(r"(?<=[.!?。])\s+", snippet)[0] if snippet else ""
        if len(first) > 100:
            first = first[:100].rstrip() + "…"
        fallback.append(f"{label}: {first or '제공된 문서 근거가 부족합니다.'}")
